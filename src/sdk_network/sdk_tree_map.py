...
def add_url_pre(pre_url, url):
    if not isinstance(pre_url, str) or not isinstance(url, str):
        raise Exception("url not str")

    if url and url[0] == "/":
        url = url[1:]

    if pre_url and pre_url[-1] == "/":
        pre_url = pre_url[:-1]

    if not url and pre_url:
        return ""

    return "%s/%s" % (pre_url, url)


def append_node_to_father_next(father, tree):
    father.next.append(tree)


def sdk_create_node_to_next(father, url, method, encode=None, decode=None, next_tree=None):
    """
    添加一个节点到已有的树的下级树列表中
    @param father:父节点
    @param url:
...
def sdk_append_node_to_root(tree):
    """
    追加树到根
    @param tree:
    @return:
    """
    if not isinstance(tree, SdkTreeMap):
...
def sdk_create_node_to_root(url, method, encode=None, decode=None, next_tree=None):

    _sdk_tree_map.append(SdkTreeMap(url, method, encode, decode, next_tree if next_tree else []))
